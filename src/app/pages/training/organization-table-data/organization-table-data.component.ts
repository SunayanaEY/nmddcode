...
export class OrganizationTableDataComponent implements OnInit {
  ...
  loadOrganizationData(): void {
    ...
    this.adminService.getOrganizationsData().subscribe({
      next: (response) => {
  ...
  deleteTrainingCentre(centre: any) {
    if (
      confirm(`Are you sure you want to delete ${centre.organizationName}?`)
    ) {
      this.adminService.deleteOrganization(centre.id).subscribe({
        next: () => {
