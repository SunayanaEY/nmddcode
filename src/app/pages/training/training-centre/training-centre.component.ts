...
export class TrainingCentreComponent implements OnInit {
  ...
  loadStates(): void {
    this.locationService.getStates().subscribe({
      next: (states) => {
        this.states = states;
        const stateField = this.modalConfig.fields?.find(field => field.id === 'newStateId');
        if (stateField) {
  ...
  loadDistrictsByState(stateId: number): void {
    this.locationService.getDistrictsByState(stateId).subscribe({
      next: (districts) => {
        ...
        const newDistrictField = this.modalConfig.fields?.find(field => field.id === 'newDistrictId');
        if (newDistrictField) {
  ...
  get totalCapacity(): number {
    return this.trainingCentres.reduce((total, centre) => total + (centre.capacity || 0), 0);
  }
  ...
  viewTrainingCentre(centre: any) {
    ...
    this.modalConfig.fields = this.modalConfig.fields?.filter(field => 
      field.id !== 'newStateId' && field.id !== 'newDistrictId'
    );
  ...
  restoreStateDistrictFields() {
    ...
    const hasNewStateField = this.modalConfig.fields?.some(field => field.id === 'newStateId');
    const hasNewDistrictField = this.modalConfig.fields?.some(field => field.id === 'newDistrictId');
    
    if (!hasNewStateField) {
      // Find the position after 'district' field to insert state change field
      const districtIndex = this.modalConfig.fields?.findIndex(field => field.id === 'district');
    ...
    if (!hasNewDistrictField) {
      ...
      const newStateIndex = this.modalConfig.fields?.findIndex(field => field.id === 'newStateId');
  ...
  getFieldDisabledState(fieldId: string, centre: any): boolean {
    ...
    if (fieldId === 'registrationId' || fieldId === 'currentState' || fieldId === 'currentDistrict') {
      return true;
    }
    
    if (fieldId === 'newDistrictId') {
      const newStateField = this.modalConfig.fields?.find(field => field.id === 'newStateId');
  ...
  performToggle() {
    ...
    this.adminService.toggleActiveInactive(this.pendingToggleItem.id).subscribe({
      next: (response) => {
        ...
        const index = this.trainingCentres.findIndex(centre => centre.id === this.pendingToggleItem.id);
        if (index !== -1) {
          this.trainingCentres[index].active = !this.trainingCentres[index].active;
        }
        ...
        const newStatus = this.trainingCentres[index].active ? 'activated' : 'deactivated';
  ...
  onModalFieldChange(event: { fieldId: string; value: any }) {
    ...
    } else if (event.fieldId === 'instituteImage' && event.value instanceof File) {
  ...
  updateTrainingCentre(updatedData: any) {
    ...
    const newStateId = Array.isArray(updatedData.newStateId) ? updatedData.newStateId[0] : updatedData.newStateId;
    const newDistrictId = Array.isArray(updatedData.newDistrictId) ? updatedData.newDistrictId[0] : updatedData.newDistrictId;
    const originalStateId = Array.isArray(this.selectedCentre.stateId) ? this.selectedCentre.stateId[0] : this.selectedCentre.stateId;
    const originalDistrictId = Array.isArray(this.selectedCentre.districtId) ? this.selectedCentre.districtId[0] : this.selectedCentre.districtId;
    ...
    this.adminService.updateTrainingInstitute(formData).subscribe({
       next: (response) => {
         // Update the item in the local array
         const index = this.trainingCentres.findIndex(centre => centre.id === this.selectedCentre.id);
         if (index !== -1) {
           // Update with the response data or merge with existing data
           this.trainingCentres[index] = { ...this.trainingCentres[index], ...response };
         }
